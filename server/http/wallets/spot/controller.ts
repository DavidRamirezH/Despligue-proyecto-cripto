...
export const controllers = {
  ...
  depositCron: handleController(async () => {
    ...
    const lastRunTime = cronLastRunTimes.spotWalletsDeposit

    // Check if at least an hour has passed since the last run
    if (now - lastRunTime < fifteenMin) {
      throw new Error('Deposit cron can only run once per 15 minutes')
    }
    try {
      await processPendingDeposits()
      cronLastRunTimes.spotWalletsDeposit = now
      return {
  ...
  withdrawCron: handleController(async () => {
    ...
    const lastRunTime = cronLastRunTimes.spotWalletsWithdraw

    // Check if at least an hour has passed since the last run
    if (now - lastRunTime < halfHour) {
      throw new Error('Withdrawal cron can only run once per 30 minutes')
    }
    try {
      await processPendingWithdrawals()
      cronLastRunTimes.spotWalletsWithdraw = now
    } catch (error) {
      throw new Error(error)
    }
  }),
