...
const collapseEnabled = computed(() => {
  return (
    app.mash.collapse?.navigation?.enabled !== false && props.collapse !== false
  )
})
const toolbarEnabled = computed(() => {
  return (
    app.mash.collapse?.toolbar?.enabled !== false && props.toolbar !== false
  )
})
const circularMenuEnabled = computed(() => {
  return (
    app.mash.collapse?.circularMenu?.enabled !== false &&
...
<template>
  <div class="bg-muted-100 dark:bg-muted-900 pb-10">
    <slot name="navigation">
      <MashCollapseNavigation v-if="collapseEnabled" />
      <div
        role="button"
    ...
    <MashCollapseCircularMenu v-if="circularMenuEnabled" />
  </div>
</template>
