...
const sidebarEnabled = computed(() => {
  return (
    app.mash.sidebar?.navigation?.enabled !== false && props.sidebar !== false
  )
})
...
const circularMenuEnabled = computed(() => {
  return (
    app.mash.sidebar?.circularMenu?.enabled !== false &&
...
<template>
  <div class="bg-muted-100 dark:bg-muted-900 pb-10">
    <slot name="sidebar">
      <MashSidebarNavigation
        v-if="sidebarEnabled"
        :subsidebar="props.subsidebar"
      >
        <div
          v-if="app.mash.sidebar?.navigation?.logo?.component"
    ...
    <MashPanels />
    <MashSidebarCircularMenu v-if="circularMenuEnabled" />
  </div>
</template>
