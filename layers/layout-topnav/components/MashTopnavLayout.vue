...
const config = useAppConfig().mash.topnav
...
<template>
  <div>
    <div class="dark:bg-muted-900 bg-muted-50 pb-10">
      <slot name="navigation">
        <MashTopnavNavigation
          v-if="topnavEnabled"
      ...
      <MashTopnavCircularMenu v-if="circularMenuEnabled" />
    </div>

    <MashTopnavFooter v-if="config?.footer?.enabled" :display="props.display" />
  </div>
</template>
